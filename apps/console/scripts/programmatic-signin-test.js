...
async function register(email, password) {
  const res = await fetchWithCookie(BASE + '/api/auth/register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, password, name: 'Prog Test' }),
  });
  return res.json();
}
...
async function signin(email, password) {
  ...
  const res = await fetchWithCookie(BASE + '/api/auth/callback/credentials', {
    method: 'POST',
...
async function generateTrial() {
  ...
  const res = await fetchWithCookie(BASE + '/api/trial/generate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-forwarded-for': String(Date.now()) },
    body: JSON.stringify({ title: 'Prog Test' }),
  });
  return res.json();
}
